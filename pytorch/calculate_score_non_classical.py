...
from utils.utilities import (create_folder, get_filename, traverse_folder, 
...
def calculate_metrics(args, thresholds=None):
    ...
    stats_list = []
    if len(stats_list) == 0:
        print("Warning: No metrics calculated. Check if inference produced results.")
        return
...
if __name__ == '__main__':
    ...
    parser_infer_prob.add_argument('--dataset', type=str, required=True, choices=['maestro', 'maps', 'non_classical'])
    ...
    parser_metrics.add_argument('--dataset', type=str, required=True, choices=['maestro', 'maps', 'non_classical'])
